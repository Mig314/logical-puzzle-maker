...
//num_peopleは登場人物の数，num_varietyは属性の数，num_matrixは表の数，stateは状態(初期は1，問題生成後は2，解答生成後は4，問題生成禁止時はその-1倍)を表します。
...
function change_button(){
    ...
    let tmp_people=Number(document.querySelector(`form`).radio_people.value);
    let tmp_variety=Number(document.querySelector(`form`).radio_variety.value);
    if(tmp_people===4){
        if(tmp_variety===4){
            button_message="（所要時間：数秒）";
        }else if(tmp_variety===5){
            button_message="（所要時間：数分）";
        }
    }else if(tmp_people===5){
        if(tmp_variety===3){
            button_message="（所要時間：数秒）";
        }else if(tmp_variety===4){
            button_message="（所要時間：数分）";
        }else if(tmp_variety>4){
            button_message="（メモリー不足のため生成できません）";
            state*=-1;
        }
    }else if(tmp_people===6){
        if(tmp_variety===3){
            button_message="（所要時間：数分）";
        }else if(tmp_variety>3){
            button_message="（メモリー不足のため生成できません）";
            state*=-1;
        }
    }else if(tmp_people===7){
        if(tmp_variety===3){
